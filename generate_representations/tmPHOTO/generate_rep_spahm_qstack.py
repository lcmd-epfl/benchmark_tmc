...
def rep_generator(rep, **kwargs):
    ...
    if rep == "SPAHM_a":
        ...
        mols = tqdm.tqdm(mols, file=sys.stdout)
        ...
        for mol, path in zip(mols, kwargs["paths"]):
            ...
            print("Each", rep_one.shape, global_rep.shape)
            reps.append(global_rep)

        reps = np.stack(reps, axis=0)
        print("Final", reps.shape)
        print(f"{rep} representations generated!")
    elif rep == "SPAHM_b":
        mols = [qstack.compound.xyz_to_mol(xyz, basis="minao", charge=charge, spin=spin, ecp="ccpvdzpp")
                for xyz, charge, spin in zip(kwargs["paths"], kwargs["charges"], kwargs["spins"])]
...
def main():
    ...
    dataset="tmPHOTO"
    df= pd.read_csv("tmPHOTO_property.csv")
    ...
    only_z = ['Fe', 'Ni', 'Cu', 'Zn', 'Ru', 'Pd', 'Ag', 'Cd', 'Re', 'Ir', 'Pt', 'Au', 'Hg']
    max_el = 902
    elements = ['Ag', 'Au', 'B', 'Br', 'C', 'Cd', 'Cl', 'Cu', 'F', 'Fe', 'H', 'Hg', 'I', 'Ir', 'N', 'Ni', 'O', 'P', 'Pd', 'Pt', 'Re', 'Ru', 'S', 'Si', 'Zn']
